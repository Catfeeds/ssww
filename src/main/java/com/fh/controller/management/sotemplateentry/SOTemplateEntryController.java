package com.fh.controller.management.sotemplateentry;
 ...
 */
@Controller
@RequestMapping(value="/sotemplateentry")
public class SOTemplateEntryController extends BaseController {
	
	String menuUrl = "sotemplateentry/list.do"; //菜单地址(权限用)
	...
	public ModelAndView save() throws Exception{
		logBefore(logger, Jurisdiction.getUsername()+"新增SOTemplateEntry");
		if(!Jurisdiction.buttonJurisdiction(menuUrl, "add")){return null;} //校验权限
		ModelAndView mv = this.getModelAndView();
		PageData pd = new PageData();
		pd = this.getPageData();
		pd.put("SOTEMPLATEENTRY_ID", this.get32UUID());	//主键
		sotemplateentryService.save(pd);
	...
	public void delete(PrintWriter out) throws Exception{
		logBefore(logger, Jurisdiction.getUsername()+"删除SOTemplateEntry");
		if(!Jurisdiction.buttonJurisdiction(menuUrl, "del")){return;} //校验权限
		PageData pd = new PageData();
		pd = this.getPageData();
		System.out.println(pd);
		sotemplateentryService.delete(pd);
		out.write("success");
		out.close();
	}
	...
	public void delete2(PrintWriter out) throws Exception{
		...
		pd = this.getPageData();
		System.out.println(pd);
		sotemplateentryService.delete2(pd);
		out.write("success");
		out.close();
	}
	...
	public ModelAndView edit() throws Exception{
		logBefore(logger, Jurisdiction.getUsername()+"修改SOTemplateEntry");
		if(!Jurisdiction.buttonJurisdiction(menuUrl, "edit")){return null;} //校验权限
		ModelAndView mv = this.getModelAndView();
		PageData pd = new PageData();
		pd = this.getPageData();
		sotemplateentryService.edit(pd);
		mv.addObject("msg","success");
		mv.setViewName("save_result");
		return mv;
	}
	...
	public ModelAndView list(Page page) throws Exception{
		...
		page.setPd(pd);
		List<PageData>	varList = sotemplateentryService.list(page);	//列出SOTemplateEntry列表
	...
	public ModelAndView goAdd()throws Exception{
		...
		PageData pd = new PageData();
		pd = this.getPageData();
		mv.setViewName("management/sotemplateentry/sotemplateentry_edit");
	...
	public ModelAndView goEdit()throws Exception{
		...
		PageData pd = new PageData();
		pd = this.getPageData();
		pd = sotemplateentryService.findById(pd);	//根据ID读取
		mv.setViewName("management/sotemplateentry/sotemplateentry_edit");
	...
	public Object deleteAll() throws Exception{
		logBefore(logger, Jurisdiction.getUsername()+"批量删除SOTemplateEntry");
		...
		if(null != DATA_IDS && !"".equals(DATA_IDS)){
			...
			sotemplateentryService.deleteAll(ArrayDATA_IDS);
			pd.put("msg", "ok");
		}else{
			pd.put("msg", "no");
		}
	...
	public ModelAndView exportExcel() throws Exception{
		logBefore(logger, Jurisdiction.getUsername()+"导出SOTemplateEntry到excel");
		...
		titles.add("备注");	//5
		dataMap.put("titles", titles);
		List<PageData> varOList = sotemplateentryService.listAll(pd);
		List<PageData> varList = new ArrayList<PageData>();
		for(int i=0;i<varOList.size();i++){
			PageData vpd = new PageData();
			...
			vpd.put("var4", varOList.get(i).get("FAUXQTY").toString());	//4
			vpd.put("var5", varOList.get(i).getString("FREMARK"));	    //5
			varList.add(vpd);
		}
		dataMap.put("varList", varList);
		ObjectExcelView erv = new ObjectExcelView();
