...
<body class="no-skin">
			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						...
						<!-- 检索  -->
						<form action="newforwx/list.do" method="post" name="Form" id="Form">
							...
							<c:choose>
								<c:when test="${not empty varList}">
									...
									<c:forEach items="${varList}" var="var" varStatus="vs">
										<tr>
											<td class='center'>
												<label class="pos-rel"><input type='checkbox' name='ids' value="${var.NEWFORWX_ID}" class="ace" /><span class="lbl"></span></label>
											</td>
											<td class='center' style="width: 30px;">${vs.index+1}</td>
											<td class='center'>${var.TITLE}</td>
											<%-- <td class='center'>${var.NEW_CONTENT}</td> --%>
											<td class='center'>暂无法预览内容</td>
											<%-- <td class='center'>${fn:substring(${var.DESCRIBE}, 0, 1)}</td> --%>
											<td class='center'>${var.DESCRIBE}</td>
											<td class='center'>${var.AUTHOR}</td>
											<td class='center'><a class='btn btn-xs btn-info' onclick="previewPic('${var.MAIN_PIC}');">预览封面图片</a></td>
											<td class="center">
	...
	<script type="text/javascript">
		...
		//导出excel
		function toExcel(){
			window.location.href='<%=basePath%>newforwx/excel.do';
		}
		...
		function add_New(){
			var diag = new top.Dialog();
			 diag.Drag=true;
			 diag.Title ="新增群发内容";
			 diag.URL = "<%=basePath%>newforwx/goAdd_New";
			diag.Width =  window.innerWidth;
			diag.Height = window.innerHeight;
			diag.Modal = true; //有无遮罩窗口
			diag.ShowMaxButton = true; //最大化按钮
		...
		function previewPic(pic_url){
			var diag = new top.Dialog();
			 diag.Drag=true;
			 diag.Title ="封面预览图片";
			 diag.URL = "<%=basePath%>newforwx/toPreview?pic_url="+pic_url;
			diag.Width =  800;
			diag.Height = 550;
			diag.Modal = true; //有无遮罩窗口
			diag.ShowMaxButton = true; //最大化按钮
