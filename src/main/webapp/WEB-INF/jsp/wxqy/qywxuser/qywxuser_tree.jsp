...
<body class="no-skin">
							...
							<table style="width:100%;" border="0">
								<tr>
									<td style="width:15%;" valign="top" bgcolor="#F9F9F9"><a
										...
										class="btn btn-light btn-xs" onclick="updateDep()">同步部门及企业用户信息<i
											id="nav-search-icon"
	...
	<script type="text/javascript">
		$(top.hangge());//关闭加载状态
		function a(){
			$("#treeFrame").attr("src","<%=basePath%>commodity/toManagecom");
		}
		/*  var setting = {   
		...
		var setting = {
			...
			callback : {
				onClick : function(event, treeId, treeNode, clickFlag) {
					...
					if(treeNode.id == 1){
						keywords ="";
						$("#treeFrame").attr("src","<%=basePath%>qywxuser/list?keywords="+keywords);
					}
					$("#treeFrame").attr("src","<%=basePath%>qywxuser/list?keywords="+keywords);
		...
		function updateDep(){
			$.ajax({  
                ...
                type:'POST',  
                //dataType:"String",  
                url:'<%=basePath%>test1/test_getDep',
				success : function(data) {
					alert(data.result) ;
